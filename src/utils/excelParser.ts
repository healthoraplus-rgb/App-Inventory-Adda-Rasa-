...
export interface IncomingExportItem {
  id: string;
  date: string;
  code: string;
  productId?: string;
...
export interface IncomingExportOptions {
  month?: string;
  supplier?: string;
  product?: string;
  startDate?: string;
...
/**
 * Generates and downloads a professional Excel Report for Incoming Inventory (Barang Masuk)
 */
export function exportIncomingReportToXLSX(
  items: IncomingExportItem[],
  options: IncomingExportOptions = {}
) {
  ...
  const sheetData: any[][] = [
    ...
    [],
    ['LAPORAN REKAPITULASI PENERIMAAN BARANG MASUK'],
    [
    ...
    [],
    // Table Column Headers (Clean, No 'Petugas')
    [
      'No',
      'Tanggal',
      'No Transaksi',
  ...
  items.forEach((item, index) => {
    ...
    sheetData.push([
      ...
      item.price,
      item.totalPrice,
      item.notes || item.sourceDestination || '-',
    ]);
  });
  ...
  const fileName = `Laporan_Barang_Masuk_${companyName.replace(/\s+/g, '_')}_${cleanMonthName}_${cleanSupplierName}.xlsx`;
...
export function exportSalesReportToXLSX(
  ...
  items.forEach((item, index) => {
    ...
    sheetData.push([
      ...
      item.price,
      item.totalPrice,
      item.notes ? `${item.sourceDestination} (${item.notes})` : item.sourceDestination,
    ]);
  });
  ...
  worksheet['!cols'] = [
    ...
    { wch: 30 }, // Tujuan / Outlet / Keterangan
  ];
...
export interface ReturnExportItem {
  id: string;
  date: string;
  code: string;
  type: 'RETUR_IN' | 'RETUR_OUT';
  productCode: string;
  productName: string;
  category: string;
  supplier: string;
...
/**
 * Generates and downloads a rich, multi-sheet Excel (.xlsx) report for Product Returns
 */
export function exportReturnsReportToXLSX(
  returns: ReturnExportItem[],
  options?: {
    filterMonth?: string;
    companyName?: string;
  }
  ...
  const companyName = options?.companyName || 'ADDA RASA KJD';
  const filterMonth = options?.filterMonth || 'Semua Periode';
  const now = new Date();
  ...
  const sheetData: any[][] = [
    [companyName.toUpperCase()],
    ['LAPORAN MUTASI RETUR BARANG (RETUR MASUK & KELUAR)'],
    [`Periode: ${filterMonth} | Tanggal Ekspor: ${printDateStr}`],
    [],
    [
      'No',
      'Tanggal',
  ...
  returns.forEach((item, index) => {
    if (item.type === 'RETUR_IN') {
      totalReturIn += item.quantity;
    } else {
      totalReturOut += item.quantity;
    }
    const itemPrice = item.price || 0;
    const itemTotal = item.totalPrice || item.quantity * itemPrice;
    totalNilaiRetur += itemTotal;

    sheetData.push([
      index + 1,
      item.date,
      item.code,
      item.type === 'RETUR_IN' ? 'RETUR MASUK (+Outlet)' : 'RETUR KELUAR (-Supplier)',
      item.productCode,
      item.productName,
      item.category,
      item.supplier,
      ...
      itemPrice,
      itemTotal,
      item.returnReason || item.notes || item.sourceDestination || '-',
    ]);
  });
  ...
  const fileName = `Laporan_Retur_Barang_${companyName.replace(/\s+/g, '_')}_${filterMonth.replace(/[^a-zA-Z0-9]/g, '_')}.xlsx`;
  XLSX.writeFile(workbook, fileName);
}
