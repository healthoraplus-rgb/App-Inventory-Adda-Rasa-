...
export const SalesExportModal: React.FC<SalesExportModalProps> = ({
  ...
  const salesItems: SalesExportItem[] = useMemo(() => {
    ...
    return outboundTransactions
      .map((t) => {
        ...
        const price = t.price || matchedProduct?.price || 0;
        const supplier =
          (matchedProduct?.supplier && matchedProduct.supplier.trim() !== '' ? matchedProduct.supplier : undefined) ||
          (t.supplier && t.supplier.trim() !== '' ? t.supplier : undefined) ||
          (suppliers.length > 0 ? suppliers[0].name : 'Adda Rasa');
  ...
  const handleExportPdf = async () => {
    ...
    setPdfStatusMessage('Mempersiapkan data...');
    setViewMode('pdfPreview');
    
    // Wait for state render to finish so printAreaRef is mounted & visible
    await new Promise((resolve) => setTimeout(resolve, 200));

    if (!printAreaRef.current) {
      setIsGeneratingPdf(false);
      setPdfStatusMessage('Gagal: Elemen dokumen belum siap.');
      return;
    }

    const docTitle = `Laporan_Penjualan_ADDA_RASA_${selectedMonth.replace(/\s+/g, '_')}`;
    try {
      await exportElementToPdf(printAreaRef.current, `${docTitle}.pdf`, (msg) => {
        setPdfStatusMessage(msg);
      });
    } catch (err) {
      console.error('Failed to export sales PDF', err);
      setPdfStatusMessage('Gagal membuat PDF.');
    } finally {
      setTimeout(() => {
        setIsGeneratingPdf(false);
        setPdfStatusMessage('');
  ...
  const handleDownloadHtmlDocument = () => {
    ...
    downloadDocumentAsHtml(
      ...
      `${docTitle}.html`,
      `Laporan Rekapitulasi Penjualan - ${selectedMonth}`
    );
  };
  ...
  const handleOpenNewTab = () => {
    ...
    openDocumentInNewTab(
      printAreaRef.current.innerHTML,
      `Laporan Penjualan (${selectedMonth}) - ADDA RASA KJD`
    );
  };
      ...
      <div className="bg-white rounded-2xl max-w-5xl w-full max-h-[92vh] flex flex-col shadow-2xl border border-[#c4c5d5]/40 overflow-hidden">
        ...
        <div className="p-4 md:p-5 border-t border-[#c4c5d5]/30 bg-[#fbf8ff] flex flex-col sm:flex-row justify-between items-center gap-3 no-print">
          <div className="text-[12px] text-[#757684]">
            {isGeneratingPdf ? (
              <span className="flex items-center gap-2 text-[#00288e] font-semibold animate-pulse">
                <span className="material-symbols-outlined text-[18px] animate-spin">progress_activity</span>
                {pdfStatusMessage || 'Memproses berkas PDF...'}
              </span>
            ) : (
              <span>Format file menyertakan Logo, Alamat, Kontak, dan Rekapitulasi resmi ADDA RASA.</span>
            )}
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2.5 w-full sm:w-auto justify-end">
            ...
            <button
              id="btn-print-sales-printer"
              type="button"
              onClick={handlePrintDocument}
              className="flex items-center justify-center gap-1.5 px-3.5 py-2.5 border border-[#00288e] text-[#00288e] hover:bg-[#00288e]/10 rounded-xl text-[13px] font-bold transition-all cursor-pointer"
              title="Cetak langsung ke mesin pencetak / printer"
            >
            ...
            <button
              ...
              className="flex items-center justify-center gap-2 px-4 py-2.5 bg-[#00288e] text-white hover:bg-[#1e40af] disabled:opacity-60 rounded-xl text-[13px] font-bold shadow-xs transition-all cursor-pointer"
              title="Download dokumen langsung sebagai file PDF (.pdf)"
            >
              {isGeneratingPdf ? (
                <>
                  <span className="material-symbols-outlined text-[18px] animate-spin">progress_activity</span>
            ...
            <button
              ...
              className="flex items-center justify-center gap-1.5 px-5 py-2.5 bg-[#00288e] text-white hover:bg-[#1e40af] rounded-xl text-[13px] font-bold shadow-xs transition-all cursor-pointer"
            >
