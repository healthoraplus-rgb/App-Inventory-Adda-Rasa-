    ...
    <div className="max-w-[1600px] mx-auto space-y-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          {(onResetStockToZero || onClearStockAndTransactions) && (
            <button
              onClick={() => setIsResetStockModalOpen(true)}
              title="Hapus atau reset data stok fisik & riwayat mutasi"
              className="flex items-center gap-2 px-3.5 py-2.5 rounded-xl font-semibold text-[13px] border border-[#ba1a1a]/40 text-[#ba1a1a] bg-white hover:bg-[#ffdad6]/30 transition-all shadow-xs cursor-pointer"
            >
              <span className="material-symbols-outlined text-[18px]">delete_sweep</span>
              <span>Hapus Data Stok</span>
            </button>
          )}
      ...
      {/* Reset & Delete Stock Modal */}
      <ResetStockModal
        isOpen={isResetStockModalOpen}
        onClose={() => setIsResetStockModalOpen(false)}
        productCount={products.length}
        transactionCount={transactions.length}
        onResetStockToZero={() => {
          if (onResetStockToZero) onResetStockToZero();
        }}
        onClearStockAndTransactions={() => {
          if (onClearStockAndTransactions) onClearStockAndTransactions();
        }}
      />
    </div>
  );
