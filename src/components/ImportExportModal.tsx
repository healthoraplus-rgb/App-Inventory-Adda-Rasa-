...
export const ImportExportModal: React.FC<ImportExportModalProps> = ({
  ...
  // Compute new unique suppliers that will be auto-synced to the Supplier module
  const knownSupplierNames = new Set(
    existingSuppliers.map((s) => s.name.trim().toLowerCase())
  );
  const newSuppliersDetected: string[] = [];
  ...
  previewItems.forEach((item) => {
    ...
    if (
      ...
      !knownSupplierNames.has(supName.toLowerCase()) &&
      !detectedSet.has(supName.toLowerCase())
    ) {
      detectedSet.add(supName.toLowerCase());
      newSuppliersDetected.push(supName);
    }
  });
          ...
          {previewItems.length > 0 && !isProcessing && (
            <div className="space-y-3">
              ...
              <div>
                ...
                {/* Auto Supplier Sync Notification */}
                {newSuppliersDetected.length > 0 && (
                  <div className="mb-2.5 p-3 bg-[#e8f0fe] rounded-xl border border-[#1a73e8]/30 flex items-start gap-2.5 text-[12px] text-[#174ea6]">
                    <span className="material-symbols-outlined text-[18px] text-[#1a73e8] shrink-0 mt-0.5">
                      sync_alt
                    </span>
                    <div>
                      <span className="font-bold block">
                        Otomatis Sinkronisasi ke Modul Supplier ({newSuppliersDetected.length} Mitra Baru Terdeteksi)
                      </span>
                      <p className="mt-0.5 text-[#1967d2]">
                        Supplier berikut belum ada di modul Supplier dan akan otomatis didaftarkan:
                      </p>
                      <div className="flex flex-wrap gap-1.5 mt-1.5">
                        {newSuppliersDetected.map((sup, sIdx) => (
                          <span
                            key={sIdx}
                            className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md bg-white border border-[#1a73e8]/30 text-[#1a73e8] font-bold text-[11px] shadow-2xs"
                          >
