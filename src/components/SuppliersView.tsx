        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredSuppliers.map((s) => {
            ...
            const actualProductCount = products.filter(
              (p) => p.supplier && p.supplier.trim().toLowerCase() === s.name.trim().toLowerCase()
            ).length;
            const countToDisplay = actualProductCount > 0 ? actualProductCount : s.productCount;
                ...
                <div className="mt-5 pt-3.5 border-t border-[#c4c5d5]/20 space-y-3">
                  <div className="flex justify-between items-center text-[12px]">
                    <div>
                      ...
                      <span className="font-bold text-[#1a1b22] ml-1">{countToDisplay} Produk</span>
                    </div>
